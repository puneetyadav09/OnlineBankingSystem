...
public class rmvservices {
    ...
    JButton submit;
    rmvservices(int formno){
        frame = new JFrame("Remove Services");

        heading = new JLabel("Remove Services");
            ...
            public void actionPerformed(ActionEvent e) {
                try{
                    ...
                    if(finalIspassbook && pass.isSelected()){
                        c.s.execute("UPDATE `userbankdetails` SET `passbook`='no' WHERE formno = " + formno);
                    }
                    if(finalIsonline && onln.isSelected()){
                        c.s.execute("UPDATE `userbankdetails` SET `onlineinternet`='no' WHERE formno = " + formno);
                    }
                    if(finalIsatm && atm.isSelected()){
                        c.s.execute("UPDATE `userbankdetails` SET `atmcard`='no' WHERE formno = " + formno);
                    }
                    if(finalIsemail && emlsms.isSelected()){
                        c.s.execute("UPDATE `userbankdetails` SET `emailsms`='no' WHERE formno = " + formno);
                    }
